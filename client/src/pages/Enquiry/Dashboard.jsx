...
const Dashboard = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const response = await apiService.put(`/records/${id}/status`, {
        status: newStatus,
      });
      if (response.data.success) {
        // Update local state
        setRecords(
          records.map((record) =>
            record.id === id
              ? { ...record, admission_status: newStatus }
              : record,
          ),
        );
  ...
  const handleSendEmail = async (id) => {
    ...
    try {
      ...
      // 3. Capture PDF
      const element = document.getElementById("resend-capture-area");
      const opt = {
        margin: 0,
      ...
      const response = await apiService.post(
        `/records/${id}/send-email`,
        payload,
      );

      if (response.data.success) {
        toast.success("Email sent successfully with PDF!", {
          id: loadingToast,
        });
        fetchRecords();
      } else {
  ...
  return (
    <div className={styles.dashboard}>
                ...
                {currentRecords.length > 0 ? (
                  currentRecords.map((record, index) => (
                    <tr key={record.id}>
                      ...
                      <td>
                        <span
                          className={`${styles.statusBadge} ${styles["email-" + (record.email_status || "Pending")]}`}
                        >
      ...
      {isGenerating && captureData && (
        ...
        >
          <div id="resend-capture-area" style={{ background: "#fff" }}>
            <RecordReport data={captureData} standalone={true} />
          </div>
        </div>
      )}
    </div>
